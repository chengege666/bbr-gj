...
system_tools_menu() {
    while true; do
        ...
        printf "${GREEN}%-40s${NC}\n" "9. 重启服务器"
        printf "${GREEN}%-40s${NC}\n" "10. 卸载本脚本"
        printf "${GREEN}%-40s${NC}\n" "11. Nginx Proxy Manager 管理"
        printf "${GREEN}%-40s${NC}\n" "12. 查看端口占用状态"
        printf "${GREEN}%-40s${NC}\n" "13. 修改DNS 服务器"
        printf "${GREEN}%-40s${NC}\n" "14. 磁盘空间分析"
        printf "${GREEN}%-40s${NC}\n" "15. 服务器性能全面测试 (Bench.sh)"
        printf "${GREEN}%-40s${NC}\n" "16. 流媒体解锁检测 (RegionRestrictionCheck)"
        printf "${GREEN}%-40s${NC}\n" "17. 回程路由测试 (BestTrace)"
        printf "${GREEN}%-40s${NC}\n" "18. 炫酷系统信息显示 (neofetch)"
        printf "${GREEN}%-40s${NC}\n" "19. 实时系统监控仪表板 (gtop/bpytop)"
        printf "${GREEN}%-40s${NC}\n" "20. 网速多节点测试 (Speedtest-X)"
        printf "${GREEN}%-40s${NC}\n" "21. 端口扫描工具 (nmap)"
        printf "${GREEN}%-40s${NC}\n" "22. 证书管理工具 (acme.sh)"
        printf "${GREEN}%-40s${NC}\n" "23. 服务器延迟测试 (Ping测试)"
...
manage_1panel() {
    while true; do
        ...
        if systemctl is-active 1panel >/dev/null 2>&1; then
            PANEL_STATUS="${GREEN}✅ 已安装并运行${NC}"
            PANEL_VERSION=$(1panel version 2>/dev/null | grep -oP 'Version:\s*\K[^\s]+' || echo "未知")
            PANEL_PORT=$(ss -tlnp | grep 1panel | grep -oP ':\K[0-9]+' | head -1 || echo "未知")
        else
            PANEL_STATUS="${RED}❌ 未安装或未运行${NC}"
            PANEL_VERSION="未知"
            PANEL_PORT="未知"
        fi
        
        echo -e "${BLUE}当前状态: $PANEL_STATUS${NC}"
        echo -e "${BLUE}版本信息: $PANEL_VERSION${NC}"
        echo -e "${BLUE}面板端口: $PANEL_PORT${NC}"
        echo ""
        echo "1. 安装 1Panel (最新版)"
        echo "2. 卸载 1Panel (完全删除)"
        echo "3. 启动 1Panel 服务"
        echo "4. 停止 1Panel 服务"
        echo "5. 重启 1Panel 服务"
        echo "6. 查看 1Panel 状态"
        echo "7. 查看 1Panel 日志"
        echo "8. 修改 1Panel 端口"
        echo "9. 重置 1Panel 管理员密码"
        echo "10. 备份 1Panel 数据"
        echo "11. 恢复 1Panel 数据"
        echo "0. 返回上级菜单"
        echo "=========================================="
        ...
        case $panel_choice in
            ...
            6) status_1panel ;;
            7) logs_1panel ;;
            8) change_1panel_port ;;
            9) reset_1panel_password ;;
            10) backup_1panel_data ;;
            11) restore_1panel_data ;;
            0) return ;;
            *) echo -e "${RED}无效的选项！${NC}"; read -p "按回车键继续..." ;;
        esac
    done
}
...
install_1panel() {
    ...
    # 检查是否已安装
    if systemctl is-active 1panel >/dev/null 2>&1; then
        echo -e "${YELLOW}⚠️ 1Panel 已经安装并运行！${NC}"
        read -p "按回车键返回..."
        return
    fi
    ...
    # 检查Docker是否安装
    if ! command -v docker >/dev/null 2>&1; then
        echo -e "${YELLOW}检测到系统未安装Docker，将自动安装Docker...${NC}"
        install_update_docker
    fi
    
    echo -e "${YELLOW}正在下载并安装 1Panel...${NC}"
    ...
    echo -e "${GREEN}✅ 开始安装 1Panel，这可能需要几分钟时间...${NC}"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "${YELLOW}访问地址: http://服务器IP:目标端口${NC}"
        echo -e "${YELLOW}默认用户名: admin${NC}"
        echo -e "${YELLOW}默认密码: 1panel${NC}"
        echo -极e "${RED}⚠️ 请及时修改默认密码！${NC}"
    else
        echo -e "${RED}❌ 1Panel 安装失败！极NC}"
    fi
...
uninstall_1panel() {
    ...
    echo -e "${RED}⚠️⚠️⚠️ 警告：此操作将完全删除 1Panel 及其数据！⚠️⚠️⚠️${NC}"
    echo -e "${YELLOW}这将删除：${NC}"
    echo "- 1Panel 程序文件"
    echo "- 1Panel 数据目录 (/opt/1panel)"
    echo "- 1Panel 服务配置"
    echo "- 所有面板设置和数据"
    
    read -p "确定要卸载 1Panel 吗？(输入 'YES' 确认): " confirm
    if [[ "$confirm" != "YES" ]]; then
    ...
    rm -rf /usr/local/bin极1panel
    rm -rf /etc/systemd/system/1panel.service
    rm -rf /opt/1panel
    rm -rf /usr/local/1panel
    
    # 删除数据目录（谨慎操作，再次确认）
    read -p "是否删除 1Panel 数据目录？这将永久丢失所有数据！(y/N): " delete_data
    if [[ "$delete_data" == "y" || "$delete_data" == "Y" ]]; then
        rm -rf /opt/1panel
        echo -e "${GREEN}✅ 数据目录已删除${NC}"
    else
        echo -e "${YELLOW}⚠️ 数据目录保留在 /opt/1panel${NC}"
    fi
...
# 启动1Panel服务
start_1panel() {
    echo -e "${YELLOW}启动 1Panel 服务...${NC}"
    systemctl start 1panel
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ 1Panel 启动成功！${NC}"
    else
        echo -e "${RED}❌ 1Panel 启动失败！${NC}"
    fi
    sleep 2
}
...
# 停止1Panel服务
stop_1panel() {
    echo -e "${YELLOW极停止 1Panel 服务...${NC}"
    systemctl stop 1panel
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ 1Panel 已停止！${NC}"
    else
        echo -e "${RED}❌ 1Panel 停止失败！${NC}"
    fi
    sleep 2
}
...
# 重启1Panel服务
restart_1panel() {
    echo -e "${YELLOW}重启 1Panel 服务...${NC}"
    systemctl restart 1panel
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ 1Panel 重启成功！${NC}"
    else
        echo -e "${RED}❌ 1Panel 重启失败！${NC}"
    fi
    sleep 2
}
...
status_1panel() {
    ...
    1panel version 2>/dev/null || echo -e "${RED}无法获取版本信息${NC}"
...
logs_极panel() {
    ...
    echo -e "${YELLOW}显示最近 100 行日志（按 Ctrl+C 退出）：${NC}"
    journalctl -u 1panel -n 100 -f
...
change_1panel_port() {
    ...
    if ! systemctl is-active 1panel >/dev/null 2>&1; then
        echo -e "${RED}❌ 1Panel 未运行，请先启动服务！${NC}"
        read -p "按回车键返回..."
        return
    fi
    
    current_port=$(ss -tlnp | grep 1panel | grep -oP ':\K[0-9]+' | head -1)
    echo -e "${BLUE}当前端口: ${YELLOW}$current_port${NC}"
    ...
    echo -e "${YELLOW}停止 1Panel 服务...${NC}"
    systemctl stop 1panel
    
    # 修改配置文件（假设使用默认配置路径）
    if [ -f /opt/1panel/conf/app.conf ]; then
        sed -i "s/^server.port=.*/server.port=$new_port/" /opt/1panel/conf/app.conf
    fi
    ...
    echo -e "${YELLOW}新的访问地址: http://服务器IP:$new_port${NC}"
    
    read -p "按回车键返回..."
}

# 重置1Panel管理员密码
reset_1panel_password() {
    clear
    echo -e "${CYAN}=========================================="
    echo "           重置 1Panel 管理员密码          "
   极=========================================="
    echo -e "${NC}"
    
    if ! command -v 1panel >/dev/null 2>&1; then
        echo -e "${RED}❌ 1Panel 未安装！${NC}"
        read -p "按回车键返回..."
        return
    fi
    
    echo -极{YELLOW}正在重置管理员密码...${NC}"
...
backup_1panel_data() {
    ...
    backup_dir="/opt/1panel_backup_$(date +%Y%m%d_%H%M%S)"
    mkdir -p "$backup_dir"
    
    echo -e "${YELLOW}正在备份 1Panel 数据...${NC}"
    
    # 备份配置文件
    if [ -d /opt/1panel/极nf ]; then
        cp -r /opt/1panel/conf "$backup_dir/"
        echo -e "${GREEN极✅ 配置文件已备份${NC}"
    fi
    
    # 备份数据文件
    if [ -d /opt/1panel/data ]; then
        cp -r /opt/1panel/data "$backup_dir/"
        echo -e "${GREEN}✅ 数据文件已备份${NC}"
    fi
    
    # 备份数据库（如果有）
    if [ -d /opt/1panel/db ]; then
        cp -r /opt/1panel/db "$backup_dir/"
    ...
    echo -e "${YELLOW}备份文件: ${backup_dir}.tar.gz${NC}"
...
restore_1panel_data() {
    ...
    echo -e "${RED}⚠️ 警告：恢复操作将覆盖现有数据！${NC}"
    read -p "确定要恢复 1Panel 数据吗？(y/N): " confirm
    if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then
    ...
    # 解压备份文件
    temp_dir="/tmp/1panel_restore_$(date +%s)"
    mkdir -p "$temp_dir"
    tar -xzf "$backup_file" -C "$temp_dir"
    
    # 恢复文件
    if [ -d "$temp_dir/conf" ]; then
        cp -r "$temp_dir/conf" /opt/1panel/
        echo -e "${GREEN}✅ 配置文件已恢复${NC}"
    fi
    
    if [ -d "$temp_dir/data" ]; then
        cp -r "$temp_dir/data" /opt/1panel/
        echo -e "${GREEN}✅ 数据文件已恢复${极C}"
    fi
    
    if [ -d "$temp_dir/db" ]; then
        cp -r "$temp_dir/db" /opt/1panel/
